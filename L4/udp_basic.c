...
{
    int ser;
    if((ser = socket(AF_INET, SOCK_DGRAM, 0)) < 0){
        ERR("socket");
    }
    int optval;
    ...
    while(1){
        ...
        int ret;
        if((ret = recvfrom(ser, buffer, sizeof(buffer), MSG_WAITALL, &client_addrss, &len)) < 0){
            ERR("recv");
        }
        if(ret == 0) break;
        if(ret > 0){
...
void client_work(struct sockaddr_in addr)
{
    sleep(2);
    int cli;
    if((cli = socket(AF_INET, SOCK_DGRAM, 0)) < 0){
        ERR("socket");
    }
    ...
    for(int i = 0; i < 10; i++){
        ...
        if(sendto(cli, msg, strlen(msg)+1, MSG_CONFIRM,(const struct sockaddr*)&addr, sizeof(addr)) < 0){
            ERR("send");
        }
        sleep(1);
    }
    // NOTE: UDP connections don't know when to stop so you have to send an indicator
    if(sendto(cli, "END", 4, MSG_CONFIRM,(const struct sockaddr*)&addr, sizeof(addr)) < 0){
        ERR("send");
    }
    sleep(1);
